    ...
    with tab2:
        ...
        if event_to_manage:
            ...
            with st.form("admin_manage_event_form"):
                ...
                # Admin creates a new sheet and provides the link
                if st.form_submit_button("Create New Google Sheet for this Event"):
                    with st.spinner("Creating new event sheet..."):
                        try:
                            new_sheet_copy = client.copy(EVENT_TEMPLATE_SPREADSHEET_KEY, title=f"Event - {event_to_manage}", copy_permissions=True)
                            sheet_link = new_sheet_copy.url
                            st.success(f"New sheet created! Link: {sheet_link}")
                            st.info("The link has been automatically added below. Click 'Save and Approve' to finalize.")
                            # Update the form with the new link
                            st.session_state.new_sheet_link = sheet_link
                        except Exception as e:
                            st.error(f"Failed to create new sheet. Ensure template is shared. Error: {e}")
                ...
                if update_button:
                    ...
                    events_sheet.update_cell(cell.row, 5, external_url) # URL
                    events_sheet.update_cell(cell.row, 8, whatsapp_link) # WhatsappLink
                    events_sheet.update_cell(cell.row, 9, sheet_link) # Project_Demo_Sheet_Link
                ...
                if approve_button:
                    if not sheet_link:
                        st.error("You must provide or create a 'Project Demo Sheet Link' before approving the event.")
                    else:
                        cell = events_sheet.find(event_to_manage)
                        events_sheet.update_cell(cell.row, 5, external_url)
